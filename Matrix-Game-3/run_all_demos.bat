...
rem Collect all demo folders
set COUNT=0
for /d %%D in (demo_images\*) do (
    set /a COUNT+=1
    set FOLDER_!COUNT!=%%~nxD
    set IMAGE_!COUNT!=%%D\image.png
    set PROMPT_FILE_!COUNT!=%%D\prompt.txt
)

rem Iterate in reverse order
for /l %%I in (!COUNT!, -1, 1) do (
    set FOLDER=!FOLDER_%%I!
    set IMAGE=!IMAGE_%%I!
    set PROMPT_FILE=!PROMPT_FILE_%%I!
